...
// 读取 JSON 文件
const filePath = path.join(__dirname, 'data', 'ENGAA_2016_S1B_Final.json');
...
mongoose.connect(process.env.MONGO_URI)
  .then(async () => {
    ...
    await Question.deleteMany({ source: 'eng_2016_s1b' });
    await MockExam.deleteMany({ source: 'eng_2016_s1b' });
    ...
    const formatted = rawQuestions.map(q => ({
      ...
      difficultyScore: 3.0,
      isMock: true,
      source: 'eng_2016_s1b', // 这里标记为 section 1B
    }));
